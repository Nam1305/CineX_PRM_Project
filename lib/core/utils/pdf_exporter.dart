  ...
  static Future<void> exportScreenplay({
    ...
    try {
      ...
      String formatDateStr(String? isoString) {
        return formatDateOnly(isoString, fallback: 'Chưa rõ');
      }
      ...
      var productionPlan = ProductionPlan.empty(project.id ?? 0);
      if (project.id != null) {
        try {
          productionPlan =
              await LocalCacheService.instance.getProductionPlan(project.id!) ??
              productionPlan;
        } catch (_) {}
        try {
          productionPlan = await ApiService().getProductionPlan(project.id!);
          try {
            await LocalCacheService.instance.upsertProductionPlan(
              productionPlan,
            );
          } catch (_) {}
        } catch (_) {
          // Keep the cached plan when the server is unavailable.
        }
      }
      ...
      final customDates = {
        for (final entry in productionPlan.locationDates.entries)
          if (locationNames[entry.key] case final name?) name: entry.value,
      };
      final sceneShootingStatuses = {
        for (final entry in productionPlan.sceneStatuses.entries)
      ...
      DateTime? getShootingDate(int dayIndex) {
        ...
        final locationName = groupedMap.keys.elementAt(dayIndex);
        final persistedDate = customDates[locationName];
        return persistedDate == null ? null : DateTime.tryParse(persistedDate);
      }

      // Tính toán thống kê dự án (tiến độ quay thực tế)
      final totalScenes = chronologicalScenes.length;
      final doneScenes = chronologicalScenes
          .where(
            (s) =>
                s.status == SceneStatus.done &&
                sceneShootingStatuses[s.id] == SceneStatus.done,
          )
          .length;
      final inProgressScenes = chronologicalScenes
          .where(
            (s) =>
                s.status == SceneStatus.done &&
                sceneShootingStatuses[s.id] == SceneStatus.inProgress,
          )
          .length;
      final todoScenes = totalScenes - doneScenes - inProgressScenes;
      final progressPercent = totalScenes == 0
          ? 0
          : (doneScenes / totalScenes * 100).round();

      final totalLocations = chronologicalScenes
          .map((s) => s.locationId)
          .whereType<int>()
          .toSet()
          .length;
      final intCount = chronologicalScenes
          .where((s) => s.setting == LocationSetting.interior)
          .length;
      final extCount = chronologicalScenes
          .where((s) => s.setting == LocationSetting.exterior)
          .length;
      final intPercent = totalScenes == 0
          ? 0
          : (intCount / totalScenes * 100).round();
      final extPercent = totalScenes == 0
          ? 0
          : (extCount / totalScenes * 100).round();
                    ...
                    child: pw.Column(
                      children: [
                        ...
                        _buildCoverMetaRow(
                          ...
                          fontRegular,
                          'Tổng quy mô:',
                          '${acts.length} hồi / ${allScenes.length} phân cảnh',
                        ),
                      ],
                    ),
                  ),
                    ...
                    children: [
                      pw.Expanded(
                        child: _buildSummaryCard(
                          ...
                          'TIẾN ĐỘ CHUNG',
                          '$progressPercent%',
                          'Đang quay: $inProgressScenes | Chờ: $todoScenes',
                        ),
                      ),
                      ...
                      pw.Expanded(
                        child: _buildSummaryCard(
                          ...
                          fontRegular,
                          'QUY MÔ NHÂN SỰ',
                          '${uniqueCharacters.length} Diễn viên',
                          'Thành viên đoàn: ${project.crewCount}',
                        ),
                      ),
                      pw.SizedBox(width: 12),
                      pw.Expanded(
                        child: _buildSummaryCard(
                          ...
                          'Nội $intCount / Ngoại $extCount cảnh',
                        ),
                      ),
                    ],
                  ),
                    ...
                    children: [
                      ...
                      if (sortedChars.isEmpty)
                        pw.TableRow(
                          children: [
                            ...
                            pw.Padding(
                              padding: const pw.EdgeInsets.all(8),
                              child: pw.Text(
                                'Chưa có nhân vật nào được phân vai',
                                style: pw.TextStyle(
                                  font: fontRegular,
                                  fontSize: 9,
                                ),
                        ...
                        ...sortedChars.asMap().entries.map((entry) {
                          ...
                          final ratio = totalScenes == 0
                              ? 0
                              : (count / totalScenes * 100).round();
                          return pw.TableRow(
                            children: [
                              pw.Padding(
                                padding: const pw.EdgeInsets.all(6),
                  ...
                  children: [
                    ...
                    pw.Table(
                      ...
                      children: [
                        ...
                        // Table Body Rows
                        ...groupedMap.entries.toList().asMap().entries.map((
                          entryIdx,
                        ) {
                          ...
                          final dateStr = date != null
                              ...
                              : '';
                          final dayLabel = dateStr.isNotEmpty
                              ? 'Ngày ${idx + 1}\n($dateStr)'
                              : 'Ngày ${idx + 1}';
                          final sceneNums = group.value
                              .map((s) => s.sceneNumber)
                              .join(', ');
          ...
          pw.MultiPage(
            ...
            build: (pw.Context context) {
              return [
                ...
                ...actScenes.map((scene) {
                  final setting =
                      scene.location?.setting == LocationSetting.interior
                      ? 'INT'
                      : 'EXT';
                  final locName = scene.location?.name ?? 'CHƯA CÓ BỐI CẢNH';
                  final time = scene.location?.timeOfDay == SceneTime.day
                      ? 'NGÀY'
                      : 'ĐÊM';
                  final charactersList = scene.characters
                      .map((c) => c.name)
                      .join(', ');
